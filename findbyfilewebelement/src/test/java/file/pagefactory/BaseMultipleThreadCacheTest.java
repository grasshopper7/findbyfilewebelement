...
public class BaseMultipleThreadCacheTest {
	 ...
	 * - Valid PageFactory initialization without mock test.(COMP) - Valid By options test
	 */
	...
	protected int checkFieldData(TestPage page, Class annot) throws IllegalArgumentException, IllegalAccessException {
		...
		getAllFields(fields, page.getClass());
		for (Field field : fields/*page.getClass().getFields()*/) {
			// Made inner class TestPage implementations static to avoid the this$0
			// reference
			...
			if (field.getAnnotation(annot) == null && field.getAnnotation(FindBy.class) == null)
				Assert.assertNull(field.getName() + " of class " + page.getClass() + " should not be initialized.",
						field.get(page));
			// FindBy annotated field should be initialized,One can also add FindBys and
			// FindAll
			if (field.getAnnotation(FindBy.class) != null)
				Assert.assertNotNull(field.getName() + " of class " + page.getClass() + " is not initialized.",
						field.get(page));
			...
			if (field.getAnnotation(annot) != null) {
				...
				Assert.assertEquals("By value stored in cache for element " + field.getName() + " of class "
						+ page.getClass() + " is not correct.", By.id(field.getName()),
						FieldByCache.getByForField(field));
			}
		}
		return count;
	}
